#!/bin/bash
# Complete TLS Message Capture Test Script
# This script runs a full TLS handshake and captures all messages
#
# Usage: ./run_test.sh [output_dir]
...
mkdir -p "$RESULTS_DIR"
OUTPUT_DIR="${1:-$RESULTS_DIR/tls_capture_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$OUTPUT_DIR"
cd "$OUTPUT_DIR"

echo "=========================================="
echo "TLS Message Capture Test"
echo "=========================================="
...
cd "$SCRIPT_DIR"
python3 tls_proxy.py 8443 localhost 4433 > "$OUTPUT_DIR/proxy.log" 2>&1 &
PROXY_PID=$!
sleep 3
